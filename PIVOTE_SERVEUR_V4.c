...
#define IDT_LOGINPWD 1002
#define IDT_CONFETTI 1003
#define IDT_DLGPWD_BASE 1100
...
/* GDI objects */
static HFONT  hFBrand, hFTitle, hFNorm, hFSub, hFBig, hFNav, hFMono;
...
#define MAX_WINNERS 16
#define CONFETTI_COUNT 72
...
static BOOL loginPwdVisible = FALSE;
static BOOL celebrationActive = FALSE;
static int  celebrationTick = 0;
static int  celebrationWinnerCount = 0;
static int  celebrationMaxVoix = 0;
static char celebrationLabel[256] = "";
static ConfettiPiece celebrationPieces[CONFETTI_COUNT];
...
static void DrawLogoMark(HDC, int, int, int);
...
static void SetPasswordMask(HWND, BOOL);
static void HideLoginPassword(void);
static void RevealLoginPassword(HWND);
static ResultSummary BuildResultSummary(void);
static void StopCelebration(HWND);
static void StartCelebration(HWND);
static void UpdateCelebration(HWND);
static void DrawCelebration(HDC, RECT*);
...
static void InitGDI(void)
{
    ...
    hFBrand = CreateFont(-30,0,0,0,FW_BOLD,0,0,0,DEFAULT_CHARSET,
                OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,
                VARIABLE_PITCH | FF_ROMAN,"Georgia");
...
static void FreeGDI(void)
{
    DeleteObject(hFBrand); DeleteObject(hFTitle); DeleteObject(hFNorm);
...
static void DrawLogoMark(HDC hdc, int x, int y, int size)
{
    int lidH = size / 5;
    int boxY = y + size / 3;
    int stroke = size / 14;
    int slotW = size / 3;
    int slotX = x + (size - slotW) / 2;
    int slotY = boxY + 6;
    int paperW = size / 3;
    int paperH = size / 3;
    RECT paper = {
    ...
    HPEN penFrame, penCheck, oldPen;
    HBRUSH brGlass, brLid, brPaper, oldBrush;
    POINT checkPts[3];

    if (stroke < 2) stroke = 2;

    brGlass = CreateSolidBrush(RGB(204, 228, 236));
    brLid   = CreateSolidBrush(RGB(122, 132, 148));
    brPaper = CreateSolidBrush(RGB(255, 250, 240));
    penFrame = CreatePen(PS_SOLID, stroke, RGB(112, 122, 138));

    oldPen = (HPEN)SelectObject(hdc, penFrame);
    oldBrush = (HBRUSH)SelectObject(hdc, brGlass);
    Rectangle(hdc, x + stroke, boxY + lidH, x + size - stroke, y + size);

    SelectObject(hdc, brLid);
    Rectangle(hdc, x, boxY, x + size, boxY + lidH);

    SelectObject(hdc, brPaper);
    Rectangle(hdc, paper.left, paper.top, paper.right, paper.bottom);

    SelectObject(hdc, oldBrush);
    SelectObject(hdc, oldPen);

    DeleteObject(brGlass);
    DeleteObject(brLid);
    DeleteObject(brPaper);
    DeleteObject(penFrame);

    DrawFilledRect(hdc, slotX, slotY, slotW, 4, RGB(42, 52, 68));

    penCheck = CreatePen(PS_SOLID, stroke, RGB(76, 180, 120));
    oldPen = (HPEN)SelectObject(hdc, penCheck);
    checkPts[0].x = paper.left + paperW / 4;
    checkPts[0].y = paper.top + paperH / 2;
    checkPts[1].x = paper.left + paperW / 2 - 2;
    checkPts[1].y = paper.top + paperH - 6;
    checkPts[2].x = paper.right - paperW / 5;
    checkPts[2].y = paper.top + 8;
    Polyline(hdc, checkPts, 3);
    SelectObject(hdc, oldPen);
    DeleteObject(penCheck);
}
...
static void SetPasswordMask(HWND hEdit, BOOL masked)
{
    if (!hEdit) return;
    SendMessage(hEdit, EM_SETPASSWORDCHAR, masked ? (WPARAM)'*' : 0, 0);
...
{
    if (!hMain) return;
    HWND hBtn = GetDlgItem(hMain, IDC_BSHOWPWD);
    KillTimer(hMain, IDT_LOGINPWD);
    SetPasswordMask(GetDlgItem(hMain, IDC_EPASS), TRUE);
    if (hBtn) SetWindowText(hBtn, "Voir 3s");
    loginPwdVisible = FALSE;
}

static void RevealLoginPassword(HWND hWnd)
{
    HWND hBtn = GetDlgItem(hWnd, IDC_BSHOWPWD);
    SetPasswordMask(GetDlgItem(hWnd, IDC_EPASS), FALSE);
    if (hBtn) SetWindowText(hBtn, "Visible");
    loginPwdVisible = TRUE;
    KillTimer(hWnd, IDT_LOGINPWD);
    SetTimer(hWnd, IDT_LOGINPWD, 3000, NULL);
}
...
static ResultSummary BuildResultSummary(void)
{
    ...
    for (int i = 0; i < nbCandidats; i++) {
        s.totalVotes += candidats[i].voix;
        if (candidats[i].voix > s.maxVoix)
            s.maxVoix = candidats[i].voix;
    }
    for (int i = 0; i < nbElecteurs; i++)
        if (electeurs[i].vote_blanc) s.blancs++;

    s.totalVotes += s.blancs;
    if (s.maxVoix > 0) {
        for (int i = 0; i < nbCandidats && s.winnerCount < MAX_WINNERS; i++)
            if (candidats[i].voix == s.maxVoix) s.winners[s.winnerCount++] = i;
    }
    return s;
}

static void StopCelebration(HWND hWnd)
{
    KillTimer(hWnd, IDT_CONFETTI);
    celebrationActive = FALSE;
    celebrationTick = 0;
    celebrationWinnerCount = 0;
    celebrationMaxVoix = 0;
    celebrationLabel[0] = '\0';
}

static void StartCelebration(HWND hWnd)
{
    static const COLORREF palette[] = {
        RGB(255, 195, 74), RGB(106, 191, 255), RGB(76, 212, 112),
        RGB(255, 102, 96), RGB(163, 113, 247), RGB(255, 255, 255)
    };
    ResultSummary s = BuildResultSummary();
    int span = (CONT_W > 120) ? CONT_W - 80 : 800;

    StopCelebration(hWnd);
    if (s.totalVotes <= 0 || s.maxVoix <= 0 || s.winnerCount <= 0) return;

    srand((unsigned)time(NULL) ^ (unsigned)GetTickCount());
    celebrationActive = TRUE;
    celebrationTick = 0;
    celebrationWinnerCount = s.winnerCount;
    celebrationMaxVoix = s.maxVoix;

    if (s.winnerCount == 1) {
        snprintf(celebrationLabel, sizeof(celebrationLabel), "Victoire de %s",
                 candidats[s.winners[0]].nom);
    } else {
        strcpy(celebrationLabel, "Egalite finale");
    }

    for (int i = 0; i < CONFETTI_COUNT; i++) {
        celebrationPieces[i].w = 6 + rand() % 8;
        celebrationPieces[i].h = 4 + rand() % 8;
        celebrationPieces[i].x = rand() % span;
        celebrationPieces[i].y = -(rand() % 220);
        celebrationPieces[i].vx = (rand() % 7) - 3;
        if (celebrationPieces[i].vx == 0) celebrationPieces[i].vx = 1;
        celebrationPieces[i].vy = 3 + rand() % 5;
        celebrationPieces[i].color = palette[rand() % (sizeof(palette)/sizeof(palette[0]))];
        celebrationPieces[i].circle = (rand() % 3) == 0;
    }

    SetTimer(hWnd, IDT_CONFETTI, 70, NULL);
}

static void UpdateCelebration(HWND hWnd)
{
    int span = (CONT_W > 120) ? CONT_W - 80 : 800;
    ...
    celebrationTick++;
    for (int i = 0; i < CONFETTI_COUNT; i++) {
        celebrationPieces[i].x += celebrationPieces[i].vx;
        celebrationPieces[i].y += celebrationPieces[i].vy;
        if ((celebrationTick % 2) == 0 && celebrationPieces[i].vy < 12)
            celebrationPieces[i].vy++;
        if (celebrationPieces[i].y > CONT_H - 30) {
            celebrationPieces[i].x = rand() % span;
            celebrationPieces[i].y = -(rand() % 80);
            celebrationPieces[i].vy = 3 + rand() % 5;
        }
    }

    if (celebrationTick > 60) {
        StopCelebration(hWnd);
    } else {
...
static void DrawCelebration(HDC hdc, RECT *rc)
{
    if (!celebrationActive) return;

    for (int i = 0; i < CONFETTI_COUNT; i++) {
        int x1 = rc->left + celebrationPieces[i].x;
        int y1 = rc->top + celebrationPieces[i].y;
        int x2 = x1 + celebrationPieces[i].w;
        int y2 = y1 + celebrationPieces[i].h;
        HBRUSH br = CreateSolidBrush(celebrationPieces[i].color);
        HPEN pen = CreatePen(PS_SOLID, 1, celebrationPieces[i].color);
        HPEN oldPen = (HPEN)SelectObject(hdc, pen);
        HBRUSH oldBrush = (HBRUSH)SelectObject(hdc, br);
        if (celebrationPieces[i].circle) Ellipse(hdc, x1, y1, x2, y2);
        else Rectangle(hdc, x1, y1, x2, y2);
        SelectObject(hdc, oldBrush);
        SelectObject(hdc, oldPen);
        DeleteObject(br);
        DeleteObject(pen);
    ...
    if (celebrationLabel[0]) {
        char details[64];
        DrawFilledRect(hdc, rc->right - 320, rc->top + 10, 296, 48, RGB(24, 30, 52));
        DrawTextAt(hdc, celebrationLabel, rc->right - 320, rc->top + 10, 296, 24,
                   C_WARN, hFSub, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        snprintf(details, sizeof(details),
                 (celebrationWinnerCount == 1) ? "%d voix" : "%d voix ex aequo",
                 celebrationMaxVoix);
        DrawTextAt(hdc, details, rc->right - 320, rc->top + 30, 296, 18,
                   C_TEXT, hFMono, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
    }
}
...
static void LayoutLoginControls(HWND hWnd)
{
    ...
    MoveWindow(GetDlgItem(hWnd, IDC_EPASS),  cx, cy + 194, 352, 40, TRUE);
    MoveWindow(GetDlgItem(hWnd, IDC_BSHOWPWD), cx + 362, cy + 194, 98, 40, TRUE);
...
static void DrawHeader(HDC hdc, RECT *rc)
{
    ...
    DrawLogoMark(hdc, rc->left + 22, rc->top + 12, 52);
    DrawTextAt(hdc, "PiVote", rc->left + 88, rc->top + 10, 220, 34,
               C_WHITE, hFBrand, DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    DrawTextAt(hdc, "Espace Administrateur", rc->left + 90, rc->top + 48, 320, 22,
...
static void DrawSidebar(HDC hdc, RECT *rc)
{
    ...
    rBrand.left = rc->left + 14;
    rBrand.top = rc->top + 18;
    rBrand.right = rc->right - 14;
    rBrand.bottom = rBrand.top + 88;
    DrawFilledRect(hdc, rBrand.left, rBrand.top, rBrand.right - rBrand.left, rBrand.bottom - rBrand.top, RGB(22, 30, 46));
    DrawFilledRect(hdc, rBrand.left, rBrand.top, rBrand.right - rBrand.left, 3, C_ACCE);
    DrawLogoMark(hdc, rBrand.left + 14, rBrand.top + 16, 42);
    DrawTextAt(hdc, "PiVote", rBrand.left + 68, rBrand.top + 14, 150, 28,
               C_WHITE, hFBrand, DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    DrawTextAt(hdc, "Systeme de vote electronique", rBrand.left + 68, rBrand.top + 46, 166, 20,
               C_SUBT, hFSub, DT_LEFT|DT_SINGLELINE);

    DrawTextAt(hdc, "NAVIGATION", rc->left+14, rc->top+116, 180, 16,
               C_SUBT, hFSub, DT_LEFT|DT_SINGLELINE);

    DrawFilledRect(hdc, rc->left+14, rc->top+136, rc->right-rc->left-28, 1, C_BORD);
}
...
{
    ResultSummary summary = BuildResultSummary();
    DrawFilledRect(hdc, rc->left, rc->top, rc->right-rc->left, rc->bottom-rc->top, C_BG);

    DrawTextAt(hdc, "RESULTATS EN DIRECT", rc->left + 24, rc->top + 10, 480, 34,
    ...
    int y = rc->top + 64;
    int rowH = 60;
    int actionY = CONT_Y + CONT_H - 48 - 18;
    int bannerH = 38;
    int reserveBottom;
    reserveBottom = actionY - (((!voteOuvert && summary.totalVotes > 0) ? bannerH + 14 : 0) + 10);
    ...
    for (int i = 0; i < nbCandidats && i < 6; i++) {
        BOOL isWinner = FALSE;
        double pct = (summary.totalVotes > 0) ? 100.0 * candidats[i].voix / summary.totalVotes : 0.0;
        int bw = (summary.totalVotes > 0) ? (int)(maxBarW * candidats[i].voix / summary.totalVotes) : 0;
        COLORREF col = palette[i % 6];

        if (!voteOuvert && summary.maxVoix > 0) {
            for (int w = 0; w < summary.winnerCount; w++) {
                if (summary.winners[w] == i) {
                    isWinner = TRUE;
                    break;
                }
            }
        ...
        if (isWinner) {
            ...
            Rectangle(hdc, bx + labelW - 2, y + 1, bx + labelW + maxBarW + 2, y + 29);
            SelectObject(hdc, oldBrush);
            SelectObject(hdc, oldPen);
            DeleteObject(pen);
        }
        ...
        y += rowH;
        if (y + rowH > reserveBottom) break;
    }

    if (nbElecteurs > 0 && y + rowH <= reserveBottom + 10) {
        double pctB = (summary.totalVotes > 0) ? 100.0 * summary.blancs / summary.totalVotes : 0.0;
        int bwB = (summary.totalVotes > 0) ? (int)(maxBarW * summary.blancs / summary.totalVotes) : 0;
        ...
        {
            char numB[48];
            sprintf(numB, "%d (%.1f%%)", summary.blancs, pctB);
            DrawTextAt(hdc, numB, bx + labelW + maxBarW + 12, y, valueW, 22,
                       C_SUBT, hFNorm, DT_LEFT|DT_SINGLELINE);
        }
    }

    if (!voteOuvert && summary.totalVotes > 0) {
        if (summary.winnerCount > 0 && summary.maxVoix > 0) {
            char gbuf[256] = "";
            int gy = actionY - bannerH - 8;
            COLORREF banner = (summary.winnerCount > 1) ? C_WARN : C_SUCC;

            if (summary.winnerCount == 1) {
                snprintf(gbuf, sizeof(gbuf), "  GAGNANT : %s  (%d voix)  ",
                         candidats[summary.winners[0]].nom, summary.maxVoix);
            } else {
                strcpy(gbuf, "  EGALITE : ");
                for (int i = 0; i < summary.winnerCount; i++) {
                    if (i > 0) strcat(gbuf, " / ");
                    strcat(gbuf, candidats[summary.winners[i]].nom);
                }
                {
                    char suffix[32];
                    snprintf(suffix, sizeof(suffix), "  (%d voix)  ", summary.maxVoix);
                    strcat(gbuf, suffix);
                }
            }

            DrawFilledRect(hdc, bx, gy, CONT_W - 48, bannerH, banner);
            DrawTextAt(hdc, gbuf, bx, gy, CONT_W - 48, bannerH,
                       C_BG, hFNav, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        }
    }

    DrawCelebration(hdc, rc);
}
static void CreateLoginControls(HWND hWnd)
{
    ...
    CreateWindow("BUTTON","Voir 3s",WS_CHILD|WS_TABSTOP,
                 0,0,88,32,hWnd,(HMENU)IDC_BSHOWPWD,hInst,NULL);
    ...
    /* Show controls */
    for (int i = IDC_EUSER; i <= IDC_BSHOWPWD; i++) {
...
static void RefreshElecList(void)
{
    ...
    for (int i = 0; i < nbElecteurs; i++) {
        char buf[192];
        snprintf(buf, sizeof(buf), "  ID:%-6d  %-25s  Login:%-15s  Vote:%s",
...
static void DlgHidePassword(HWND hDlg, int idx)
{
    if (idx < 0 || idx >= gCtx.nFields || !gCtx.isPwd[idx]) return;
    KillTimer(hDlg, IDT_DLGPWD_BASE + idx);
    SetPasswordMask(gCtx.hEdits[idx], TRUE);
    if (gCtx.hShowBtns[idx]) SetWindowText(gCtx.hShowBtns[idx], "Voir 3s");
    gCtx.pwdVisible[idx] = 0;
}

static void DlgRevealPassword(HWND hDlg, int idx)
{
    if (idx < 0 || idx >= gCtx.nFields || !gCtx.isPwd[idx]) return;
    SetPasswordMask(gCtx.hEdits[idx], FALSE);
    if (gCtx.hShowBtns[idx]) SetWindowText(gCtx.hShowBtns[idx], "Visible");
    gCtx.pwdVisible[idx] = 1;
    KillTimer(hDlg, IDT_DLGPWD_BASE + idx);
    SetTimer(hDlg, IDT_DLGPWD_BASE + idx, 3000, NULL);
}
    ...
    switch(msg) {
        case WM_CREATE: {
            ...
            for (int i = 0; i < gCtx.nFields; i++) {
                ...
                if (gCtx.isPwd[i]) editW -= 92;
                gCtx.hEdits[i] = CreateWindow("EDIT","",es,
                    16,y,editW,36,hDlg,(HMENU)(UINT_PTR)(900+i),hInst,NULL);
                SendMessage(gCtx.hEdits[i], WM_SETFONT, (WPARAM)hFNorm, TRUE);
                if (gCtx.isPwd[i]) {
                    gCtx.hShowBtns[i] = CreateWindow("BUTTON","Voir 3s",WS_CHILD|WS_VISIBLE|WS_TABSTOP,
                        16 + editW + 8, y, 84, 36, hDlg, (HMENU)(UINT_PTR)(950+i), hInst, NULL);
                    SendMessage(gCtx.hShowBtns[i], WM_SETFONT, (WPARAM)hFSub, TRUE);
                } else {
                    gCtx.hShowBtns[i] = NULL;
                }
                y += 44;
            }
            y += 8;
        ...
        case WM_COMMAND: {
            ...
            } else if (id >= 950 && id < 955) {
                DlgRevealPassword(hDlg, id - 950);
            } else if (id == IDCANCEL) {
        ...
        case WM_TIMER:
            if (wp >= IDT_DLGPWD_BASE && wp < IDT_DLGPWD_BASE + 5) {
                DlgHidePassword(hDlg, (int)(wp - IDT_DLGPWD_BASE));
                return 0;
            }
            break;
        case WM_KEYDOWN:
...
static void DoAddVoter(HWND hWnd)
{
    ...
    const char *lbl[] = {"Nom complet :","Login (identifiant) :","Mot de passe :"};
    const int   pwd[] = {0,0,1};
    char res[3][128];
    int id;
    char msg[256];
    if (!ShowInputDialog(hWnd,"Ajouter un electeur",3,lbl,pwd,res)) return;

    if (!res[0][0] || !res[1][0] || !res[2][0]) {
        MessageBox(hWnd,"Tous les champs sont obligatoires.","Erreur",MB_OK|MB_ICONERROR);
        return;
    }

    id = genererIdElecteurAleatoire();
    if (id <= 0) {
        MessageBox(hWnd,"Impossible de generer un ID de vote unique.","Erreur",MB_OK|MB_ICONERROR);
        return;
    }

    AuthStatus st = auth_register_user(CSV_PATH, res[1], res[2], "votant");
    ...
    Electeur e = {0};
    e.id = id;
    strncpy(e.nom, res[0], 49); e.nom[49]='\0';
    strncpy(e.username, res[1], AUTH_MAX_USERNAME); e.username[AUTH_MAX_USERNAME]='\0';
    electeurs[nbElecteurs++] = e;
    sauvegarderDonnees();
    RefreshElecList();
    InvalidateRect(hWnd, NULL, TRUE);
    snprintf(msg, sizeof(msg),
             "Electeur ajoute avec succes.\nID de vote genere : %d\nCommuniquez cet ID au votant.",
             id);
    MessageBox(hWnd, msg, "PIVOTE", MB_OK|MB_ICONINFORMATION);
}
...
static void DrawOwnerButton(DRAWITEMSTRUCT *dis)
{
    ...
    if (id >= IDC_NAV0 && id <= IDC_NAV5) {
        ...
        if (navIdx == activePanel && screenMode == 1) {
            bg = C_NSEL; fg = C_WHITE;
        } else {
            bg = pressed ? RGB(26,40,62) : C_SIDE; fg = RGB(230,238,250);
        }
        DrawFilledRect(hdc, r.left, r.top, r.right-r.left, r.bottom-r.top, bg);
        /* Barre d'accent gauche si sÃƒÂ©lectionnÃƒÂ© */
        if (navIdx == activePanel && screenMode == 1) {
            DrawFilledRect(hdc, r.left, r.top, 4, r.bottom-r.top, C_ACCE);
        }
        DrawFilledRect(hdc, r.left + 14, r.bottom - 1, r.right-r.left-28, 1,
                       (navIdx == activePanel) ? RGB(132,198,255) : RGB(34,44,66));
        DrawTextAt(hdc, navLabels[navIdx], r.left+22, r.top,
                   r.right-r.left-30, r.bottom-r.top, fg, hFNav,
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    switch(msg) {
        ...
        case WM_CREATE:
            ...
            SendMessage(GetDlgItem(hWnd,IDC_BSHOWPWD),WM_SETFONT,(WPARAM)hFSub,FALSE);
        ...
        case WM_PAINT: {
            ...
            if (screenMode == 0) {
                ...
                DrawTextAt(hdc, panTitles[activePanel],
                           CONT_X+20, CONT_Y+8, CONT_W-40, 34,
                           C_ACCE, hFTitle, DT_LEFT|DT_VCENTER|DT_SINGLELINE);
                DrawFilledRect(hdc, CONT_X+20, CONT_Y+48, CONT_W-40, 1, C_BORD);
        ...
        case WM_COMMAND: {
            ...
            if (screenMode == 0) {
                ...
                } else if (id == IDC_BSHOWPWD) {
                    RevealLoginPassword(hWnd);
                }
            } else {
        ...
        case WM_TIMER:
            if (wp == IDT_LOGINPWD && loginPwdVisible) {
                HideLoginPassword();
            } else if (wp == IDT_CONFETTI) {
                UpdateCelebration(hWnd);
            } else if (wp == IDT_REFRESH) {
